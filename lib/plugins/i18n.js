...
i18n.prototype.upsertRecord = function(id, value, language){
    ...
    this.storage.update({_id:id}, {$set: updateObject}, {}, function(err, docNum){
        if(docNum === 0){
            updateObject['_id'] = id;
            updateObject['contentType'] = self.contentType;
            self.storage.insert(updateObject);
        }
    });
    contentDep.changed();
}
...
i18n.prototype.getRecord = function(label){
    ...
    if(currentLabel){
        if(this.language in currentLabel){
            ...
            return Utilities.firstField(currentLabel) + '('+this.language+')';
        }
    } else {
...
i18n.prototype.getRecordField = function(recordId, field){
    ...
    if(record && field in record){
        if(this.language in record[field]){
            return record[field][this.language];
        } else {
            return Utilities.firstField(record[field]) + '('+this.language+')';
        }
    } else {
        return this.defaultEmptyString;
    }
